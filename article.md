...
There is a whole dedicated [piece](https://blog.swmansion.com/testing-starknet-contracts-made-easy-with-protostar-2ecdad3c9133) explaining almost everything you would need to know before reading this article, so make sure to check it out. Also, some knowledge about [EIP-712](https://eips.ethereum.org/EIPS/eip-712) would be nice too, as I won't be going into details of how it works.

# Use case - creating a binding between ETH adress and Starknet Address


## Creating a connection
On the surface the concept might be simple, we get Ethereum address and as an input and use caller's address as Starknet address, we call `save_connected_addresses` function and we're done, right? Well not really, first we have to make sure that that the Ethereum address  actually belongs to the person calling this function. Thats the hard part, we need to recreate the message signed on Ethereum side and then by using the signature given as an input we need to confirm that the address extracted from all this matches our input. Thats what `assert_valid_eth_signature` function is doing. By calling `get_caller_address()` and using it as our Starknet address we ensure that it indeed belongs to the user and we will use it when reconstructing signed message.
```cairo
@external
func add_connection{
    syscall_ptr : felt*,
...
So we will have to write `get_hash` function for ourselves.
```cairo
...
### Constants
Instead of calculating structure hash every time we can precalculate it and store it as a constant to save us some difficult operations. Additionally Ethereum uses a pre-chosen prefix which is explained in the [documentation](https://eips.ethereum.org/EIPS/eip-712)
```cairo
const PREFIX = 0x1901;
...
func set_domain_separator{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(domain_separator: Uint256){
 ...
 Since our example is pretty easy, our structure contains only one field - StarknetAddress recreating the hashable string is pretty easy, we just need to concatenate structure hash with our value.
...
The last remaining thing is to do is to combine domain hash and message hash and apply keccak256 to it. There is a small problem - the mandatory 2 byte prefix which prevents us from using the same, simplified keccak implementation. But first we need to add the prefix.
```cairo
    let prefix = PREFIX;
...
```
If we now used one of the available keccak implementation we would run into a problem - since we need to hash 66 bytes instead of 64 we must let the hashing function know to only take last 2 bytes of our `overflow` (other bytes are all 0). The easiest way to do it is to change our input to little endian format.
...
```
All that's left to do is calling `verify_eth_signature_uint256` in our `assert_valid_eth_signature` function.
```cairo
    verify_eth_signature_uint256{keccak_ptr=keccak_ptr}(hash_uint, r, s, v, eth_address);
...
```
We need to call `finaliza_keccak` because of an internal batching done
...
`func storage(eth_address : felt, starknet_address : felt*)` which instead of creating a 1-1 connection between Ethereum address and Starknet address, maps Ethereum address to an array of Starknet addresses. Of course then it's getters will need to be changed. 
